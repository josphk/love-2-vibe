...
function Player.new()
    ...
    self.maxLives = MAX_LIVES
    self.score = 0
    self.graze = 0
    self.shield = 0

    self.beamDamage = BEAM_DAMAGE
    self.speed = SPEED
    self.invulnTime = INVULN_TIME

    self.aimGX = 12    -- aim target in grid coords
    self.aimGY = 10
    self.aimAngle = -math.pi / 2

    self.currentWeapon = 2   -- 2=shotgun, 3=smg, 4=rifle (chrono always LMB/RMB)
    self.invulnTimer = 0
    self.dead = false
    return self
end
        ...
        if not e.dead then
            for _, seg in ipairs(segments) do
                ...
                if d <= BEAM_WIDTH + e.radius then
                    local killed = e:takeDamage(self.beamDamage)
                    hit = hit + 1
