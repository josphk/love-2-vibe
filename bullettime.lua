-- bullettime.lua
-- Manages the global time-scale, bullet-time meter, and visual tint.
...
--- Add meter (e.g. from kills or grazes).
...
--- Draw screen-space overlay (blue tint, vignette).
function BulletTime:drawOverlay(screenW, screenH)
    ...
    love.graphics.rectangle("fill", 0, 0, screenW, screenH * 0.12)
    -- bottom
    love.graphics.rectangle("fill", 0, screenH * 0.88, screenW, screenH * 0.12)
    -- left
    love.graphics.rectangle("fill", 0, 0, screenW * 0.08, screenH)
    -- right
    love.graphics.rectangle("fill", screenW * 0.92, 0, screenW * 0.08, screenH)
end
